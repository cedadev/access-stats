var templates = {
    ...
    usersTableBody: '<tr><th scope="row">{{user}}</th><td>{{country}}</td><td>{{institute}}</td><td>{{field}}</td><td>{{methods}}</td><td>{{datasets}}</td><td>{{accesses}}</td><td>{{size}}</td><td>{{activitydays}}</td></tr>',
    usersTableFooter: '<tr><th scope="row">{{totals}}</th><th>{{country}}</th><th>{{institute}}</th><th>{{field}}</th><th>{{methods}}</th><th>{{datasets}}</th><th>{{accesses}}</th><th>{{size}}</th><th>{{activitydays}}</th></tr>',
    traceTableBody: '{{log}}\n',
